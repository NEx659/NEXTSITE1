...
$updateObj = Get-Content -Path "scratch\rungaroon_update.json" -Raw -Encoding UTF8 | ConvertFrom-Json
$companyUpdate = $updateObj.company
$strategyJs = $updateObj.strategyJs

# 1. Update js/app.js
$appContent = Get-Content -Path "js\app.js" -Raw -Encoding UTF8
if ($appContent -notmatch "'comp-udon-50':\s*\{") {
    $appContent = $appContent -replace "('comp-udon-10':\s*\{[\s\S]*?\n\s*\}\n\s*,)", "$strategyJs`n  `$1"
    Set-Content -Path "js\app.js" -Value $appContent -Encoding UTF8
    Write-Output "Successfully added comp-udon-50 strategy to js/app.js"
} else {
    Write-Output "comp-udon-50 already present in js/app.js"
}
...
for ($i = 0; $i -lt $companies.Count; $i++) {
    if ($companies[$i].id -eq "comp-udon-50") {
        $companies[$i] = $companyUpdate
        $updated = $true
        Write-Output "Replaced comp-udon-50 in company list."
        break
    }
}

if (-not $updated) {
    Write-Output "comp-udon-50 was not found in array, appending it..."
