$updateObj = Get-Content -Encoding UTF8 -Path "scratch/comp49_update.json" -Raw | ConvertFrom-Json
...
for ($i = 0; $i -lt $dataList.Count; $i++) {
    if ($dataList[$i].id -eq "comp-udon-49") {
...
if ($found) {
    ...
    Write-Output "Successfully updated js/data.js for comp-udon-49"
} else {
    Write-Error "comp-udon-49 not found in js/data.js"
}
