...
$updateObj = Get-Content -Path "scratch\ehouse_update_final.json" -Raw -Encoding UTF8 | ConvertFrom-Json
...
if ($appContent -notmatch "'comp-udon-17':\s*\{") {
    $appContent = $appContent -replace "('comp-udon-20':\s*\{[\s\S]*?\n\s*\}\n\s*,)", "$strategyJs`n  `$1"
    Set-Content -Path "js\app.js" -Value $appContent -Encoding UTF8
    Write-Output "Successfully added comp-udon-17 strategy to js/app.js"
} else {
    Write-Output "comp-udon-17 already present in js/app.js"
}
...
for ($i = 0; $i -lt $companies.Count; $i++) {
    if ($companies[$i].id -eq "comp-udon-17") {
        $companies[$i] = $companyUpdate
        $updated = $true
        Write-Output "Replaced comp-udon-17 in company list."
        break
    }
}

if (-not $updated) {
    Write-Output "comp-udon-17 was not found in array, appending it..."
