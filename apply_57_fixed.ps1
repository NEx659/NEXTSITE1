$updateObj = Get-Content -Encoding UTF8 -Path "scratch/comp57_fixed.json" -Raw | ConvertFrom-Json
...
for ($i = 0; $i -lt $dataList.Count; $i++) {
    if ($dataList[$i].id -eq "comp-udon-57") {
...
if ($found) {
    ...
    Write-Output "Successfully updated js/data.js for comp-udon-57 with verified direct permalinks"
} else {
    Write-Error "comp-udon-57 not found in js/data.js"
}
