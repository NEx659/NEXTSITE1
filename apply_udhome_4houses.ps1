...
$updateObj = Get-Content -Path "scratch/udhome_update_4houses.json" -Raw -Encoding UTF8 | ConvertFrom-Json
...
for ($i = 0; $i -lt $arr.Count; $i++) {
    if ($arr[$i].id -eq "comp-udon-11") {
        $arr[$i] = $compUpdate
        $replaced = $true
        Write-Output "Replaced comp-udon-11 with 4 residential projects at index $i"
        break
    }
}
if (-not $replaced) {
