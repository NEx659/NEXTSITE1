...
foreach ($item in $items) {
    ...
    if ($str -match 'มารีญาก่อสร้าง' -or $str -match 'mariacons' -or $str -match 'มารีญา') {
        $url = $item.url
...
Write-Output "Total unique posts found for Mariya Construction: $($matched.Count)"
...
[System.IO.File]::WriteAllText("$PSScriptRoot/mariya_posts_clean.txt", $sb.ToString(), [System.Text.Encoding]::UTF8)
$matched | ConvertTo-Json -Depth 5 | Out-File -Encoding UTF8 "$PSScriptRoot/mariya_posts.json"
Write-Output "Saved to scratch/mariya_posts_clean.txt and scratch/mariya_posts.json"
