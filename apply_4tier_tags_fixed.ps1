...
for ($i = 0; $i -lt $companies.Count; $i++) {
    ...
    if ($hasScg -or $i -lt 12) {
        $tagVal = "strategic"
        $countStrategic++
    } elseif ($growthRate -ge 40 -or $projCount -ge 2 -or $comp.id -in @('comp-udon-54', 'comp-udon-37', 'comp-udon-58', 'comp-udon-53', 'comp-udon-32', 'comp-udon-27', 'comp-udon-21', 'comp-udon-41')) {
        $tagVal = "growth"
        $countGrowth++
    } elseif ($projCount -gt 0 -or $i -lt 38) {
        $tagVal = "opportunity"
        $countOpportunity++
    } else {
        $tagVal = "prospect"
...
Write-Host "Updated 58 companies with Add-Member!"
