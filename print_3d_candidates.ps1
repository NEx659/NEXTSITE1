...
for ($i=0; $i -lt $currentPosts.Count; $i++) {
    $p = $currentPosts[$i]
    $t = if ($p.text) { [string]$p.text } elseif ($p.message) { [string]$p.message } else { '' }
    $u = if ($p.user) { $p.user.name } else { $p.pageName }
    
    $is3D = ($t -match "(?:ภาพ|รูป|แบบ|โมเดล|งานออกแบบ|แปลน)\s*3[dD]|3[dD]\s*(?:perspective|render|ภาพ|รูป|แบบ)|perspective|render|ภาพจำลอง|แบบแปลน|ขึ้นภาพ\s*3[dD]")
    $hasRealTask = ($t -match "เทคาน|เทพื้น|ขุดฐานราก|เทตอม่อ|ยกเสาเอก|ลงเสาเข็ม|ตอกเสาเข็ม|มุงหลังคา|ก่ออิฐ|ฉาบปูน|ส่งมอบบ้าน|ส่งมอบงาน|งานติดตั้งสุขภัณฑ์|งานทาสีโครงเหล็ก|งานปูกระเบื้อง|งานฝ้า|งานเดินระบบ|งานติดบัว|งานติดอุปกรณ์ไฟฟ้า")
    
    if ($is3D -and -not $hasRealTask) {
        $l1 = ($t -split "\r?\n")[0]
        Write-Host "Candidate #$($i+1) | เพจ: $u | $l1"
    }
}
