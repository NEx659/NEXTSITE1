﻿# Test full pipeline with footer stripped across ALL steps
...
function Test-IsOtherProvince($p) {
    ...
    foreach ($prov in $otherProvinces) {
        ...
        if ($body -match $provRegex -and $body -notmatch 'อุดร') {
            return $true
        }
    }
    foreach ($dist in $outsideDistricts) {
        $distRegex = '(?:อ\.|อำเภอ|หน้างาน|พิกัด)\s*[:\s]*' + [regex]::Escape($dist)
        if ($body -match $distRegex -and $body -notmatch 'อุดร') {
            return $true
        }
    }
...
function Test-IsAd($p) {
    ...
    if ($isAd) {
        ...
        if ($hasSiteTask -and ($body -match 'อุดร' -or $body -match '(?:อ\.|อำเภอ)\s*เมือง')) {
            return $false
        }
...
Write-Host ('Total Valid Posts After Strict Footer-Stripped Pipeline: ' + $valid.Count)
$ud = $valid | Where-Object { $_.pageName -match 'UD\.Home' }
Write-Host ('UD.Home Valid Count: ' + $ud.Count)
foreach ($u in $ud) {
    Write-Host ('- ' + ($u.text -replace '\s+', ' ').Substring(0, 100))
}