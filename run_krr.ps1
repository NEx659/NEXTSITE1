...
$krr = $pa | Where-Object { $_.user.name -like "*เกียรติรุ่งเรือง*" -or $_.text -like "*เกียรติรุ่งเรือง*" }
Write-Output "KRR count in pa_posts.json: $($krr.Count)"
...
Write-Output "Written $($krr.Count) posts to scratch/krr_clean_summary.txt"
