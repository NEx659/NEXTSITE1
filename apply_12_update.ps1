$updateObj = Get-Content -Encoding UTF8 -Path "scratch/comp12_update.json" -Raw | ConvertFrom-Json
...
for ($i = 0; $i -lt $dataList.Count; $i++) {
    if ($dataList[$i].id -eq "comp-udon-12") {
...
if ($found) {
    ...
    Write-Output "Successfully updated js/data.js for comp-udon-12"
} else {
    Write-Error "comp-udon-12 not found in js/data.js"
}
