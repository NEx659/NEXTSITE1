...
$updateObj = Get-Content -Path "scratch\by_update.json" -Raw -Encoding UTF8 | ConvertFrom-Json
$companyUpdate = $updateObj.company
$strategyJs = $updateObj.strategyJs

# 1. Update js/app.js
$appContent = Get-Content -Path "js\app.js" -Raw -Encoding UTF8
if ($appContent -notmatch "'comp-udon-31':\s*\{") {
    $appContent = $appContent -replace "('comp-udon-20':\s*\{[\s\S]*?\n\s*\}\n\s*,)", "$strategyJs`n  `$1"
    Set-Content -Path "js\app.js" -Value $appContent -Encoding UTF8
    Write-Output "Successfully updated js/app.js with comp-udon-31"
} else {
    Write-Output "comp-udon-31 already present in js/app.js"
}
...
for ($i = 0; $i -lt $companies.Count; $i++) {
    if ($companies[$i].id -eq "comp-udon-31") {
        $companies[$i] = $companyUpdate
        Write-Output "Replaced comp-udon-31 in company list."
        break
    }
}
