...
$udonDistricts = @(
    "เมืองอุดรธานี", "เมืองอุดร", "อ.เมือง", "อำเภอเมือง", "ต.บ้านจั่น", "ต.หนองบัว", "ต.หมากแข้ง", "ต.บ้านเลื่อม", "ต.หมูม่น", "ต.เชียงยืน", "เชียงยืน", "หนองใส", "หนองประจักษ์", "สามพร้าว", "นาดี", "บ้านตาด", "นิคมสงเคราะห์", "กุดสระ",
...
# Explicit Ad/Promo words (Strict ads / selling floorplans / loans)
$adPromoKeywords = @(
...
function Check-HasUdonDistrictOrPersonName($text) {
    # Check 20 Udon districts
    foreach ($dist in $udonDistricts) {
        if ($text -match [regex]::Escape($dist)) {
            return @{ hasMatch = $true; reason = "พบพื้นที่/อำเภอ: $dist" }
        }
    }
    
    # Check Person Name / Customer Name
    ...
    return @{ hasMatch = $false; reason = "ไม่พบอำเภอในอุดร และไม่พบชื่อคน" }
}
...
Write-Host "============================================="
Write-Host "TOTAL POSTS CHECKED: $($step2Posts.Count)"
Write-Host "EXCLUDED AD/PROMO POSTS: $($excludedAdPosts.Count)"
Write-Host "KEPT AD/PROMO (WITH DISTRICT/PERSON NAME): $($keptAdPostsWithException.Count)"
Write-Host "KEPT REGULAR POSTS: $($nonAdPosts.Count)"
Write-Host "TOTAL KEPT AFTER STEP 3: $($keptAdPostsWithException.Count + $nonAdPosts.Count)"
Write-Host "============================================="

for ($k = 0; $k -lt $excludedAdPosts.Count; $k++) {
    $item = $excludedAdPosts[$k]
    Write-Host "---------------------------------------------"
    Write-Host "[$($k+1)] Index $($item.index) | เพจ: $($item.pageName) | คำโฆษณา: $($item.adKeyword)"
