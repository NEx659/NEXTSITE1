...
# Strict Ad/Promo Patterns (แบบบ้านโฆษณา / โปรโมชัน / สินเชื่อ / แคตตาล็อก)
$adPromoKeywords = @(
...
function Check-HasUdonDistrictOrPersonName($text) {
    ...
    # 2. Person/Customer Name patterns
    ...
    if ($text -match "(?:ท่านอาจารย์|อาจารย์|คุณหมอ|หมอ|ผอ\.|เสี่ย|ช่าง)\s*([ก-๙a-zA-Z0-9]+)?") {
        return @{ hasMatch = $true; reason = "พบชื่อบุคคล: $($matches[0])" }
    }

    if ($text -match "(?:คุณ)\s*([ก-๙a-zA-Z]+)") {
        $candidate = $matches[0]
        if ($candidate -notmatch "คุณภาพ|คุณสมบัติ|คุ้นเคย|คุณค่า|คุ้มค่า") {
            return @{ hasMatch = $true; reason = "พบชื่อบุคคล: $candidate" }
        }
    }
...
Write-Host "============================================="
Write-Host "TOTAL POSTS BEFORE STEP 3: $($step2Posts.Count)"
Write-Host "EXCLUDED ADS/PROMO (NO DISTRICT & NO PERSON): $($excludedAdPosts.Count)"
Write-Host "REMAINING POSTS AFTER STEP 3: $($keptPosts.Count)"
Write-Host "============================================="
