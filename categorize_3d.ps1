    ...
    foreach ($line in $lines) {
        ...
        if ($trimLine -match "พิกัด|หน้างาน|สถานที่ก่อสร้าง|ที่ตั้งโครงการ|โลเคชั่น|location") {
            foreach ($prov in $otherProvincesList) {
                ...
                if ($trimLine -match $provRegex -and $trimLine -notmatch "อุดร" -and -not $hasClearUdon) { return $true }
            }
            foreach ($dist in $outsideDistrictsList) {
                if ($trimLine -match "(?:อ\.|อำเภอ)\s*$dist" -and $trimLine -notmatch "อุดร" -and -not $hasClearUdon) { return $true }
            }
        }
    }
    $bodyText = $text
...
foreach ($p in $currentPosts) {
    ...
    # Check 3D keywords
    $is3D = ($text -match "(?:ภาพ|รูป|แบบ|โมเดล|งานออกแบบ|แปลน)\s*3[dD]|3[dD]\s*(?:perspective|render|ภาพ|รูป|แบบ)|perspective|render|ภาพจำลอง|แบบแปลน|ขึ้นภาพ\s*3[dD]")
    
    if ($is3D) {
        # Check if actual physical construction work is being updated on site
        $hasSiteTask = ($text -match "เทคาน|เทพื้น|ขุดฐานราก|เทตอม่อ|ยกเสาเอก|ลงเสาเข็ม|ตอกเสาเข็ม|มุงหลังคา|ก่ออิฐ|ฉาบปูน|ส่งมอบบ้าน|ส่งมอบงาน|งานติดตั้งสุขภัณฑ์|งานทาสีโครงเหล็ก|งานปูกระเบื้อง|งานฝ้า|งานเดินระบบ")
        
        if (-not $hasSiteTask) {
            $pure3D += $p
        } else {
            $realWorkWith3DText += $p
        }
    }
}

Write-Host "=== PURE 3D / GRAPHIC DESIGNS (No real site task): $($pure3D.Count) posts ==="
$idx = 1
foreach ($p in $pure3D) {
    $u = if ($p.user) { $p.user.name } else { $p.pageName }
    $l1 = ($p.text -split "\r?\n")[0]
    Write-Host "[$idx] เพจ: $u"
    Write-Host "     URL: $($p.url)"
    Write-Host "     เนื้อหา: $l1"
    $idx++
}

Write-Host "`n=== REAL SITE POSTS with 3D mention in blurb: $($realWorkWith3DText.Count) posts ==="
$idx = 1
foreach ($p in $realWorkWith3DText) {
    $u = if ($p.user) { $p.user.name } else { $p.pageName }
    $l1 = ($p.text -split "\r?\n")[0]
    Write-Host "[$idx] เพจ: $u"
    Write-Host "     URL: $($p.url)"
