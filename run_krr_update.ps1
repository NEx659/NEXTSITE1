...
$updateObj = Get-Content -Path "scratch\krr_update.json" -Raw -Encoding UTF8 | ConvertFrom-Json
...
if ($appContent -notmatch "'comp-udon-56':\s*\{") {
    $appContent = $appContent -replace "('comp-udon-42':\s*\{[\s\S]*?\n\s*\}\n\s*,)", "$strategyJs`n  `$1"
    Set-Content -Path "js\app.js" -Value $appContent -Encoding UTF8
    Write-Output "Successfully updated js/app.js with comp-udon-56"
} else {
