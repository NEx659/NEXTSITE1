$updateData = [System.IO.File]::ReadAllText("c:/Users/pannipan/Downloads/N/scratch/sd_update.json", [System.Text.Encoding]::UTF8) | ConvertFrom-Json
...
for ($i = 0; $i -lt $companies.Count; $i++) {
    if ($companies[$i].id -eq 'comp-udon-53') {
        Write-Host "Replacing index $i -> comp-udon-53 ($($updateData.name))"
        $updateData | Add-Member -NotePropertyName "id" -NotePropertyValue "comp-udon-53" -Force
...
Write-Host "Successfully replaced comp-udon-53 object with 5 active projects!"
