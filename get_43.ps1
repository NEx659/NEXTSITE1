...
$item = $dataList | Where-Object { $_.id -eq "comp-udon-43" }
$item | ConvertTo-Json -Depth 6 | Out-File -Encoding utf8 "scratch/comp43_current.json"
...
$posts = $dataset | Where-Object { 
    $_.facebookUrl -eq $fbUrl -or $_.inputUrl -eq $fbUrl -or $_.url -like "*$($item.id)*"
}
Write-Output ("Matching posts in dataset: " + $posts.Count)
$posts | ConvertTo-Json -Depth 5 | Out-File -Encoding utf8 "scratch/lakam_posts.json"
