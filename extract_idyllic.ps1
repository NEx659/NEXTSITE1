    ...
    if (Test-Path $f) {
        ...
        if ($json -is [array]) {
            foreach ($item in $json) {
                if ($item.pageName -like '*idyllic*' -or $item.facebookUrl -like '*idyllic*' -or $item.url -like '*idyllic*') {
                    $allPosts += $item
                }
            }
        }
...
Set-Content -Path 'scratch/idyllic_posts.json' -Value ($sorted | ConvertTo-Json -Depth 5) -Encoding UTF8
Write-Output "Found $($sorted.Count) unique posts for IDYLLIC Construction."
...
Set-Content -Path 'scratch/idyllic_10posts.txt' -Value $out -Encoding UTF8
Write-Output "Written posts to scratch/idyllic_10posts.txt"
