...
foreach ($comp in $companies) {
    ...
    for ($idx = 0; $idx -lt $compPosts.Count; $idx++) {
        ...
        if ($textLower.Contains('ยกเสาเอก') -or $textLower.Contains('เสาเข็ม') -or $textLower.Contains('ตอกเสา')) {
            $stageKey = 'groundbreak'
            $stageText = 'พิธียกเสาเอกและวางผังเริ่มงานก่อสร้าง'
            $prog = 15
        } elseif ($textLower.Contains('ฐานราก') -or $textLower.Contains('คานคอดิน') -or $textLower.Contains('ตอม่อ') -or $textLower.Contains('เทลีน')) {
            $stageKey = 'foundation'
            $stageText = 'งานฐานราก ตอม่อ และคานคอดิน'
            $prog = 35
        } elseif ($textLower.Contains('ส่งมอบ') -or $textLower.Contains('ทาสี') -or $textLower.Contains('ปูกระเบื้อง') -or $textLower.Contains('ตรวจงาน') -or $textLower.Contains('สุขภัณฑ์')) {
            $stageKey = 'finishing'
        ...
        $firstLine = if ($lines.Count -gt 0) { $lines[0].Trim() } else { "อัปเดตหน้างาน $($comp.name)" }
        $projTitle = if ($firstLine.Length -gt 55) { $firstLine.Substring(0, 55) + '...' } else { $firstLine }
        ...
        $projObj = [pscustomobject]@{
            ...
            name = $projTitle
            location = "อ.$($comp.district) จ.อุดรธานี"
...
Write-Host "Total projects injected across 58 companies: $totalInjected"

# Format to JSON with proper indent
