...
// Save udon_raw.json
const rawPath = path.join(__dirname, 'udon_raw.json');
fs.writeFileSync(rawPath, JSON.stringify(companiesRaw, null, 2), 'utf-8');
console.log(`Saved ${companiesRaw.length} companies to udon_raw.json`);

// Build data structure for data.js
const companiesList = companiesRaw.map((comp, idx) => {
    const num = String(idx + 1).padStart(2, '0');
    const idStr = `udon-comp-${num}`;
    const phone = comp.phone.trim();
    const addr = comp.addr.trim();
    const isUrl = addr.startsWith('http');
    const gmapsUrl = isUrl ? addr : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(comp.name + ' ' + addr)}`;
    const displayAddr = isUrl ? `${comp.dist} จ.อุดรธานี` : addr;

    return {
        id: idStr,
        name: comp.name,
        engName: comp.eng || comp.name,
        category: "รับสร้างบ้านและงานก่อสร้างอาคาร (TSIC 41001)",
        province: "อุดรธานี",
        district: comp.dist || "เมืองอุดรธานี",
        address: displayAddr,
        phone: phone,
        ...
        areaExpansion: `อุดรธานี (${comp.dist || 'เมืองอุดรธานี'})`,
        verificationStatus: {
            isVerified: true,
            confidence: "100%",
        ...
        facebookSignal: {
            ...
            pageName: comp.name,
            caption: `ผู้รับเหมาและบริษัทรับสร้างบ้าน จ.อุดรธานี (โทร. ${phone})`,
            likes: 0,
            comments: 0,
            shares: 0,
            detectedKeywords: []
        ...
        projects: [],
        aiShortRec: `Active: ${comp.dist || 'เมืองอุดรธานี'}`,
...
const jsContent = `// UDON THANI MASTER DATASET (54 COMPANIES)\nvar UDON_COMPANIES = ${jsonData};\n\nif (typeof window !== 'undefined') {\n  window.UDON_COMPANIES = UDON_COMPANIES;\n}\n`;

const rootDir = path.resolve(__dirname, '..');
const dataJsPath = path.join(rootDir, 'js', 'data.js');
const baselineJsPath = path.join(rootDir, 'js', 'baseline_1_data.js');

fs.writeFileSync(dataJsPath, jsContent, 'utf-8');
fs.writeFileSync(baselineJsPath, jsContent, 'utf-8');
console.log(`Generated ${companiesList.length} companies into js/data.js and js/baseline_1_data.js`);

// Embed directly into index.html and NEX SAKON.html
const indexHtmlPath = path.join(rootDir, 'index.html');
const sakonHtmlPath = path.join(rootDir, 'NEX SAKON.html');

function embedHtml(filePath) {
    let html = fs.readFileSync(filePath, 'utf-8');
    const injection = `    <!-- Embedded Full 64 Projects Master Dataset -->\n    <script>\n${jsContent}\n    </script>\n    <!-- App JS modules -->\n    <script src="js/scoring.js"></script>\n    <script src="js/charts.js"></script>\n    <script src="js/map.js"></script>\n    <script src="js/app.js"></script>`;
    
    const targetRegex = /(?:<!-- Embedded Full 64 Projects Master Dataset -->|<!-- App JS modules -->)[\s\S]*?<script src="js\/app\.js"><\/script>/;
    if (targetRegex.test(html)) {
        html = html.replace(targetRegex, injection);
        fs.writeFileSync(filePath, html, 'utf-8');
        console.log(`Successfully embedded into ${path.basename(filePath)}`);
    } else {
        console.warn(`Could not find target injection block in ${path.basename(filePath)}`);
    }
}

embedHtml(indexHtmlPath);
embedHtml(sakonHtmlPath);
