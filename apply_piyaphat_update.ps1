$updateData = [System.IO.File]::ReadAllText("c:/Users/pannipan/Downloads/N/scratch/piyaphat_update.json", [System.Text.Encoding]::UTF8) | ConvertFrom-Json
...
for ($i = 0; $i -lt $companies.Count; $i++) {
    if ($companies[$i].id -eq 'comp-udon-37') {
        Write-Host "Updating index $i -> comp-udon-37 ($($updateData.name))"
...
Write-Host "Successfully saved comp-udon-37 with 6 projects!"
