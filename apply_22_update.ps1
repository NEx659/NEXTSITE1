$updateObj = Get-Content -Encoding UTF8 -Path "scratch/comp22_update.json" -Raw | ConvertFrom-Json
...
for ($i = 0; $i -lt $dataList.Count; $i++) {
    if ($dataList[$i].id -eq "comp-udon-22") {
...
if ($found) {
    ...
    Write-Output "Successfully updated js/data.js for comp-udon-22"
} else {
    Write-Error "comp-udon-22 not found in js/data.js"
}
