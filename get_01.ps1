...
$item = $dataList | Where-Object { $_.id -eq "comp-udon-01" }
$item | ConvertTo-Json -Depth 6 | Out-File -Encoding utf8 "scratch/comp01_current.json"
...
$posts = $dataset | Where-Object { 
    $_.facebookUrl -eq $fbUrl -or $_.inputUrl -eq $fbUrl -or $_.url -like "*lecrown*" -or $_.url -like "*LeCrown*" -or $_.url -like "*le_crown*"
}
Write-Output ("Matching posts in dataset: " + $posts.Count)
$posts | ConvertTo-Json -Depth 5 | Out-File -Encoding utf8 "scratch/lecrown_posts.json"
